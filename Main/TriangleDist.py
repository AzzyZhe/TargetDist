...
def recover2Boxes(boxes, pts, dists):
    # 输入: boxes: [(xywh), ...] pts: [((xy), dist), ...] // 
    # 输入: boxes: [(xywh), ...] pts: [(xy), ...] dists: [dist, ...]
    # 输出: [dist, ...] 顺序同 boxes
    # 暴力匹配 O(NM)
    ret = [] 
    return ret
...
def getPointDistances(last_frame, now_frame): # 为了对应libviso2 实际应该设计一个函数传入为Matches和Pose
...
last_image = None
def deal1frame(image, boxes): # todo 匹配不成功时也应有返回的值？
    ...
    if last_image is not None:
        ...
        for obj in boxes:
            ...
            lconf.append(conf)
        # print('GetPDistBegin')
        pts, dists = getPointDistances(last_image, image)
        # print('RecoverBegin')
        bdists = recover2Boxes(pboxes, pts, dists)
        # print('RecoverDone')
        ret = zip(pboxes, lcls, lconf, bdists)
    # print(f'{ret=}')
    # last_image = image
...
def test2():
    ...
    debug_device = R"E:\Project\AdvProj_UAVVision\MyCode\MISC\vid3-cars.mp4"
...
def test3():
    ...
    try:
        while True:
            ...
            boxes = TargetDetect.deal1frame(now_frame)
            ret = deal1frame(now_frame, boxes)
            img = DistDraw.deal1frame(now_frame,ret)
